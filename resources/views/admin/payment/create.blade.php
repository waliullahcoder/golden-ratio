...
@section('content')
    <div class="row g-3 align-items-end">
        ...
        <div class="col-sm-6">
            ...
            <select class="select form-select" id="deposit_type" name="deposit_type" data-placeholder="Select Deposit Type"
                ...
                @foreach ($allDepositTypes as $key => $value)
                    <option value="{{ $key }}" {{ old('deposit_type') == $key ? 'selected' : '' }}>
                        {{ $value }}</option>
                @endforeach
            </select>
        </div>
        <div class="col-sm-6">
            ...
            <input type="text" class="form-control" id="total_invest" name="total_invest"
                value="{{ old('total_invest') }}" placeholder="Total Invest" readonly>
        </div>
        <div class="col-sm-6">
            ...
            <input type="text" class="form-control" id="total_payable" name="total_payable" placeholder="Total Payable"
                value="{{ old('total_payable') }}" readonly>
        </div>
        <div class="col-sm-6">
        ...
        <div class="col-sm-6">
            ...
            <input type="number" class="form-control" id="amount" name="amount" placeholder="Request Amount"
                max="{{ old('total_payable') ?? 0 }}" value="{{ old('amount') }}" required>
        </div>
    </div>
@endsection
