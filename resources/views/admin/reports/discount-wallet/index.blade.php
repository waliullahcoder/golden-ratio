...
@section('content')
    <table class="dataTable table table-bordered w-100">
        ...
        <tbody>
            ...
            @foreach ($statements as $row)
                ...
                <tr class="text-nowrap">
                    ...
                    <td class="text-end">{{ number_format($row->type == 'credit' ? $row->amount : 0, 2) }}</td>
                    <td class="text-end">{{ number_format($row->type == 'debit' ? abs($row->amount) : 0, 2) }}</td>
        ...
        <tfoot>
            <tr class="bg-primary">
                <th class="text-end text-white" colspan="4">Total</th>
                <th class="text-end text-white">{{ number_format($statements->where('type', 'credit')->sum('amount'), 2) }}
                </th>
                <th class="text-end text-white">
                    {{ number_format(abs($statements->where('type', 'debit')->sum('amount')), 2) }}</th>
                <th class="text-end text-white">{{ number_format($balance, 2) }}</th>
            </tr>
        </tfoot>
    </table>
@endsection
