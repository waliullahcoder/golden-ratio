   ...
   public function create(Request $request)
            ...
            DB::transaction(function () use ($request) {
             $coa = CoaSetup::where('head_name', 'Income')->where('head_type', 'I')->first();
                ...
                foreach ($request->product_id as $product_id) {
                    $discount = $request->total_amount > 0 
                        ? ($request->discount / $request->total_amount) * $request->amount[$product_id]
                        : 0.00;
                    ...
                    SalesList::create([
                        ...
                        'commission' => $request->commission[$product_id],
                        'commission_amount' => $request->commission[$product_id] * $request->qty[$product_id],
                ...
                if ($request->sale_type == 'Cash') {
                    $collection_data = Collection::withTrashed()
                        ->select(['payment_no'])
                    ...
                    $payment_no = $collection_data 
                        ? "CC" . ((int)str_replace("CC", '', $collection_data->payment_no) + 1) 
                        : "CC" . date('ym') . '001';
    ...
    public function edit(Request $request, string $id)
    {
        ...
        if ($request->ajax()) {
            ...
            $stock += SalesList::where('sales_id', $id)
                ->where('store_id', $request->store_id)
                ->where('product_edition_id', $request->product_edition_id)
                ->sum('qty');
        ...
        // Fetch sales
        $data = Sales::with('list.product')->findOrFail($id);
        $client = Client::findOrFail($data->client_id);
        $sales = $this->model::whereNot('id', $id)->where('client_id', $data->client_id)->sum('net_amount');
        $collections = Collection::whereNot('sales_id', $id)
            ->where('client_id', $data->client_id)
            ->whereIn('collection_type',['Payment','Advance'])->sum('amount');
        ...
        return view('admin.sales.edit', compact('data','additionalData'));
    }
    ...
    public function update(Request $request, string $id)
    {
        ...
        try {
            DB::transaction(function () use ($request, $id) {
                ...
                // ----------- REVERSE EXISTING DATA -----------
                $payment = Collection::withTrashed()
                            ->where('sales_id', $id)
                            ->where('remarks','Cash Sales')
                            ->first();

                if ($payment) {
                    AccountTransactionAuto::withTrashed()
                        ->where('voucher_no',$payment->payment_no)
                ...
                AccountTransactionAuto::withTrashed()
                    ->where('voucher_no',$data->invoice)
                    ->where('voucher_type','Client Sales')
                    ->forceDelete();
                ...
                // ----------- END UPDATE MAIN SALE -----------

                // ----------- CREATE/UPDATE SALES LIST -----------
                foreach ($request->product_id as $key => $product_id) {
                    ...
                    $rate       = $request->rate[$product_id];
                    $amount     = $request->amount[$product_id];
                    $discount   = $request->total_amount > 0 ? ($request->discount / $request->total_amount) * $amount : 0.00;
                    ...
                    SalesList::create([
                        ...
                        'commission_amount' => $commission * $qty,
                        'rate'       => $rate,
                        'qty'        => $qty,
                        'amount'     => $amount,
                        'discount'   => $discount,
                ...
                if ($request->sale_type == 'Cash') {
                    ...
                    $payment_no = $collection_data ? "CC".((int)str_replace("CC",'',$collection_data->payment_no)+1) : "CC".date('ym').'001';
